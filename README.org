...
Press =RET= or click with the mouse on one target: a file-content row in tool
output, a plain path reference, a local Markdown link, an HTTP(S) Markdown
link, or a bare HTTP(S) URL.  Web links open through Emacs' =browse-url=; file
links retain normal file visiting.  Tool headers, fences, preview hints, and other
non-content rows do not open files.  Plain locations such as
=src/app.el:12:3= visit their one-based physical file line and optional
one-based column; a range such as =src/app.el#L12-L20= visits its first line
only.  If an explicit location is outside an existing narrowing, the default
=widen-automatically= policy widens the file.  Coordinates apply only to
file-visiting buffers; directory targets retain native Dired point and marks.
A target without a location keeps the point, mark, and narrowing chosen by
...
can also compact automatically when needed.

Use =C-c C-b= or =M-x pi-coding-agent-sessions= to open the Pi Agent
Ledger, a sortable cross-project view of live and recent sessions.  Use
=C-c C-j= or =M-x pi-coding-agent-switch-session= for a fast minibuffer
switcher.  Sessions are grouped using =project.el='s canonical project
roots, with working and newly completed sessions first inside each
project.  Related checkouts are grouped through VC's
=known-other-working-trees= backend operation.  Project subheadings
separate the groups, while the directory column shows the session's
actual checkout or worktree.  Remote projects include their TRAMP route in
ledger and minibuffer group labels, so identically named projects on different
hosts remain distinct.  Live remote sessions appear immediately; press =g= in
the ledger to explicitly scan their remote history.  Passive activity updates
reuse cached history and do not contact remote hosts.  In the ledger, =S=
cycles project, attention, and recent group ordering.  The chat header shows
global active and newly completed counts; click that indicator to open the
ledger.

The header line also shows context usage and changes face when it
crosses the configured warning and error thresholds.
...
| =C-c C-b=        | input   | 🗂️ Open the cross-project Agent Ledger        |
| =C-c C-j=        | input   | 🔀 Switch sessions across projects            |
...
| =C-c C-b=        | chat    | 🗂️ Open the cross-project Agent Ledger        |
| =C-c C-j=        | chat    | 🔀 Switch sessions across projects            |
